<?php
	...
	if ($connection->connect_errno !=0) {
		...
		if ($alfa > 0) {	
            ...
            $players = "0:0:0:0";
            if ($passcodeSet == true) {
                $sql = 'INSERT INTO game(serverName, gamePasscode, gameStatus, activePlayer, time, timeForTour, tour, baseMoney,maxPlayers,players,place,cards,money, whosTour,movesCodes,islands,wealth) VALUES (?,?,0,0,?,?,0,?,?,?,"0:0:0:0","000:000:000:000",?,0,"0:0:0:0","0:0:0:0",?)';
                $stmt = $connection -> prepare($sql);
                $stmt -> bind_param("ssiiiisss", $name, $passcode, $gameTime, $roundTime, $money, $gamePlayer, $players, $PmoneyR, $PmoneyR);
                $stmt -> execute();
                $stmt -> close();
            } else {
                $sql = 'INSERT INTO game(serverName, gameStatus, activePlayer, time, timeForTour, tour, baseMoney,maxPlayers,players,place,cards,money, whosTour,movesCodes,islands,wealth) VALUES (?,0,0,?,?,0,?,?,?,"0:0:0:0","000:000:000:000",?,0,"0:0:0:0","0:0:0:0",?)';
                $stmt = $connection -> prepare($sql);
